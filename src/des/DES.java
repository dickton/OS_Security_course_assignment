...
public class DES
{
    ...
    /**
     * Wrapper for CBCMode() for bytes input, specifying encryption.
     */
    public long[] CBCEncrypt(byte[] plainTexts, byte[] key, byte[] IV){
        return CBCMode(bytesToLongs.bytes2longArr(plainTexts),bytesToLongs.bytes2long(key),
                bytesToLongs.bytes2long(IV),true);
    }

    /**
     * Wrapper for CBCMode(), specifying decryption.
     */
